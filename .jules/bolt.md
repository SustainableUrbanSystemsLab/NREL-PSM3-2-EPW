...
## 2026-03-23 - Pandas DataFrame mixed types vs pure numeric values.tolist()
**Learning:** In a previous optimization, `df.values.tolist()` was used to dramatically speed up CSV writing for pure numeric pandas DataFrames compared to `df.to_csv()`. However, if the DataFrame contains even a single column with mixed types or strings alongside numbers (as is the case with EPW files containing strings like observation flags), calling `.values` forces NumPy to find a common denominator type and allocate a massive object-dtype array. This negates all the performance benefits, creating significant memory overhead and actually slowing down extraction compared to just using `.to_csv()`, which handles mixed types efficiently internally in C.
**Action:** Always check the dtypes of a DataFrame before attempting to optimize serialization using raw NumPy array extraction like `.values`. If the DataFrame is guaranteed to be purely numerical (int/float), `.values.tolist()` is faster. If the DataFrame contains mixed types or strings, stick with `df.to_csv()` to prevent catastrophic object-array allocations.
